...
class TeamgameController extends AbstractController {
    ...
    private function buildTeamgameForm(TeamgameDE $teamgame, LeagueDE $league, EventDE $event) : FormInterface {
        ...
        while ($newTeamOnePlayers-- > 0) {
            $teamgame->getPlayers()->add(new TeamgameplayerDE(new PlayerDE($this->em), 1));
        }
        ...
        if ($event->getTeamsorplayerspergame() > 1) {
            ...
            while ($newTeamTwoPlayers-- > 0) {
                $teamgame->getPlayers()->add(new TeamgameplayerDE(new PlayerDE($this->em), 2));
            }
        }
        $builder = $this->createFormBuilder($teamgame)
            ...
            ->add('teamOnePlayerIdsCollection', CollectionType::class, ['entry_type' => ChoiceType::class, 'entry_options' => [ 'choices' => $playerChoices, 'attr' => ['style' => 'height: 45px;', 'class' => 'form-control'],],]);
            
            if ($event->getTeamsorplayerspergame() > 1) {
            $builder
                ->add('teamtwo', TextType::class, array('label' => 'Team Two Name', 'required' => false, 'attr' => array('class' => 'form-control', 'style' => 'height: 45px;')))
                ->add('teamTwoPlayerIdsCollection', CollectionType::class, ['entry_type' => ChoiceType::class, 'entry_options' => [ 'choices' => $playerChoices, 'attr' => ['style' => 'height: 45px;', 'class' => 'form-control'],],]);
        } 
        $builder
            ->add('save', SubmitType::class, array('label' => 'Save', 'disabled' => $disableSaveButton, 'attr' => array('class' => 'btn btn-primary mt-3', 'style' => 'margin-top: 2em;')));
	 ...
	 */
	#[Route('/teamgame/edit/{event_id}/{teamgame_id}/{gamenumber}', name: 'teamgame_edit', methods: ['POST'])]
	#[IsGranted('ROLE_ADMIN')]
    ...
    public function new(Request $request, int $event_id): Response {
        ...
        $teamGame = new TeamgameDE($event, $this->em);
        $form = $this-> buildTeamgameForm($teamGame, $league, $event);
        $form->handleRequest($request);
    ...
    /**
     * Update player collection as appropropriate 
     * 
     * @param TeamgameDE $teamgame
     * @param int[] $playerIds
     * @param int $teamNumber
     */
    private function updatePlayerCollection(TeamgameDE $teamgame, array $playerIds, int $teamNumber): void {
