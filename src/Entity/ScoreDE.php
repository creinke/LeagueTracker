...
#[ORM\Index( columns: ["player_id"], name: "fk_score_player_id" )]
#[ORM\Index( columns: ["tee_id"], name: "fk_score_tee_id" )]
class ScoreDE {
	#[ORM\Id]
	...
	public function __construct(?string $scores = null) {
		...
		$this->setVersion(1);
		if ($scores) {
			$this->setStrokes(self::pack($scores));
		}
	}
	...
	public function adjustStrokes(int $handicap, int $scoresRecorded): string {
		...
		for ($holeOffset = 0; $holeOffset < 9; $holeOffset++) {
			...
			$adjustedStrokes .= (string) $adjustedStrokesOnHole . ', ';
		}
